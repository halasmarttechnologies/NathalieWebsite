...
export default function Navbar({
  ...
  return (
    <header className="relative z-50 w-full bg-[#0a0b0f] border-b border-[#c5a059]/40 shadow-[0_6px_25px_rgba(0,0,0,0.7)] py-3.5 sm:py-4 px-4 sm:px-8 lg:px-12 transition-all">
      <div className="max-w-[1280px] mx-auto flex items-center justify-between">
        {/* Brand Logo & Tagline - Clean & Distinct */}
        <Link href="/" className="group flex flex-col items-start select-none">
          <span className="font-serif text-2xl sm:text-[27px] lg:text-[30px] font-normal text-[#ffffff] tracking-wide transition-colors group-hover:text-[#f8ebd0] drop-shadow-[0_1px_3px_rgba(0,0,0,0.8)] leading-tight">
            {isAr ? "ناتالي روزنبلوم" : "Nathalie Rosenblum"}
          </span>
          <span className="font-serif text-[14px] sm:text-[15.5px] text-[#e5be70] tracking-normal -mt-0.5 font-normal drop-shadow-[0_1px_2px_rgba(0,0,0,0.7)]">
            {isAr ? "أخصائية نفسية مرخصة في دبي" : "Licensed Psychologist in Dubai"}
          </span>
        </Link>

        {/* Desktop Navigation Links - Distinct, Clean & Neat */}
        <nav className="hidden lg:flex items-center space-x-7 xl:space-x-9 rtl:space-x-reverse">
          {navLinks.map((item) => {
            const isActive =
              item.href === "/"
                ? pathname === "/"
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`relative font-serif text-[15px] tracking-wider transition-all duration-200 py-1.5 drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)] ${
                  isActive
              ...
              >
                {item.label}
                {isActive && (
                  <span className="absolute -bottom-1 left-0 right-0 h-[2px] bg-gradient-to-r from-[#e5be70] via-[#f7e4b5] to-[#c5a059] shadow-[0_0_8px_rgba(229,190,112,0.9)]" />
                )}
              </Link>
            );
          })}
        ...
        <div className="flex items-center space-x-3 sm:space-x-4 rtl:space-x-reverse">
          ...
          <button
            ...
            className="lg:hidden p-2 rounded-md text-[#e5be70] bg-[#161720] border border-[#d4af62]/50 hover:text-white hover:bg-black/70 transition-colors focus:outline-none"
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="lg:hidden mt-3 p-5 rounded-xl bg-[#0c0d12]/98 backdrop-blur-xl border border-[#d4af62]/40 shadow-2xl flex flex-col space-y-3 animate-in fade-in slide-in-from-top-2">
          {navLinks.map((item) => (
            <Link
              key={item.href}
              href={item.href}
